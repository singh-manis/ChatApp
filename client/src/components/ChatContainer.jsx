...
const ChatContainer = () => {
  ...
  return selectedUser ?  (
        ...
        {messages.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse' }`}>
              ...
              {msg.image ? (
                ...
                <p className={`p-2 max-w-[200px] md:text-sm font-loght rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId=== authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
              )}
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      ...
      <p className='text-lg font-medium text-white text-white '>Chat Anytime ,Anywhere</p>
    </div>
  )
}
